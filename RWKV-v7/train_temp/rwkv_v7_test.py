...
if not HF_MODE:
    ...
    MODEL_NAME = "/9950backfile/zjy_2/RWKV-Depth-recurrence/RWKV-v7/train_temp/out/L400-D1024-x070/rwkv-20"
    ...
    from src.model_sharedlayers_v2 import RWKV_shared, RWKV_shared_inference
...
with torch.no_grad():
    ...
    # out = model.forward(torch.tensor(input).reshape(1,-1).cuda(), None)
    out = model.forward(input, None)
    # print(f'\nOutput:\n{out}')
    print(f'\nOutput shape 0:\n{out[0].shape}') # torch.Size([65536])
    print(f'\nOutput shape 1:\n{len(out[1])}') # length: 24
    ...
    for i in range(len(indices)):
        ...
        print(token, f'[token_id {token_id}][probability {token_prob:.2%}]')
