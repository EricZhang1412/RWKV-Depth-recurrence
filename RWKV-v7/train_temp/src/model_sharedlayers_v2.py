...
if 'x070' in os.environ["RWKV_MY_TESTING"]:
    ...
    load(
        ...
        sources=[
            f'/9950backfile/zjy_2/RWKV-Depth-recurrence/RWKV-v7/train_temp/cuda/wkv7_cuda.cu', 
            '/9950backfile/zjy_2/RWKV-Depth-recurrence/RWKV-v7/train_temp/cuda/wkv7_op.cpp'
        ], 
...
class RWKV_Tmix_x070_v2(MyModule):
    def __init__(self, args, group_id, loops_per_group):
        ...
        with torch.no_grad():
            ratio_0_to_1 = group_id * loops_per_group / (args.num_hidden_layers - 1)  # 0 to 1
            ratio_1_to_almost0 = 1.0 - (group_id * loops_per_group / args.num_hidden_layers)  # 1 to ~0
...
class BlockGroup(nn.Module):
    def __init__(self, args):
        ...
        loops_per_group = args.num_hidden_layers // args.inner_group_num
        self.rwkv_layers = nn.ModuleList(
                [
                    Block_v2(args, i, loops_per_group) 
    ...
    def forward(
        ...
        # for layer_idx, rwkv_layer in enumerate(self.rwkv_layers):
...
class RWKV_shared(pl.LightningModule):
    def __init__(self, args):
        ...
        self.rwkv_layer_groups = nn.ModuleList([BlockGroup(args) for i in range(args.num_hidden_groups)])
    ...
    def forward(
        ...
        for i in range(args.num_hidden_layers):
            ...
            outputs, out_looptimes  = self.rwkv_layer_groups[group_idx](x, v_first, output_x=output_x, output_v_first=output_v_first)
            x_states, v_first_states = outputs[0], outputs[1]
            n_nograd, n_grad = out_looptimes[0], out_looptimes[1]
...
"""
        ...
        for n in self.state_dict():
            ...
            if "ln_" in n or ".ln" in n or "time_" in n or "_mask" in n or "pos_emb" in n or '.mask.' in n or n.endswith('_w') or n.endswith('_w1') or n.endswith('_w2') or n.endswith('_bias') or (".weight" not in n):
                if 'ln_x.weight' in n:
                    layer_scale = (1+int(n.split('.')[1])) / self.args.num_hidden_layers
...
if os.environ.get('RWKV_CUDA_ON') == '1':
    from torch.utils.cpp_extension import load
    current_path = "/9950backfile/zjy_2/RWKV-Depth-recurrence/RWKV-v7/train_temp"
    load(
        name="wkv7s", 
        sources=[
